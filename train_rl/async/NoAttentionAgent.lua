...
local NoAttentionAgent = classic.class("NoAttentionAgent")
...
function NoAttentionAgent:_init(opt, policyNet, global_theta, atomic, sharedG, global_theta_mutex_id)
	-- test
	self.total 		= 0
	self.num 		= 0
	self.timetic 	= 0
	...
	log.info('Initializing NoAttentionAgent')
	...
	-- End The Net -----------------------------------------------------
	log.info('NoAttentionAgent Initialized')
	classic.strict(self)
end

function NoAttentionAgent:start()
	-- update the buffer
...
function NoAttentionAgent:takeAction( action )
	action = self.discrete and action or action:totable()
	local reward, rawObservation, terminal = self.env:step(action)
...
function NoAttentionAgent:AccumulateGradients( terminal, observation )
	local R = 0
...
function NoAttentionAgent:ApplyGradients()
...
function NoAttentionAgent:learn()
	local total_steps_to_go = self.total_steps - self.now_step
	log.info('NoAttentionAgent starting | remaining steps = %d', total_steps_to_go)
	...
	repeat
		...
		repeat
			...
			if not self.discrete then
				...
				self.reg_actions[self.batchIdx] = torch.multinomial(self.action_mus[self.batchIdx],1):squeeze()
			end

			reward, terminal, observation = self:takeAction(self.reg_actions[self.batchIdx])
		...
		if self.isTerminal:get() < 0 then
			self:saveState()
			self.env:cleanUp()
			log.info('NoAttentionAgent suspends learning at step = %d, remaining = %d', self.now_step, self.total_steps - self.now_step)
			return
		end
	...
	log.info('NoAttentionAgent ended learning steps = %d', self.total_steps)
	-- End training ----------------------------------------------------
	self.env:cleanUp()
end

-- supervising the updating norm
function NoAttentionAgent:reportNorm()
...
function NoAttentionAgent:reportReward()
...
function NoAttentionAgent:reportTDErr()
...
function NoAttentionAgent:reportSigma()
	local idx = self.statistic.statisticSteps
...
function NoAttentionAgent:reportMu()
	local idx = self.statistic.statisticSteps
...
function NoAttentionAgent:reportActionProb()
	local idx = self.statistic.statisticSteps
	local plot = Plot()
	for i = 1, self.action_num do
...
function NoAttentionAgent:progress(total_steps_to_go)
	self.atomic:inc()
	...
	if self.now_step % self.progFreq == 0 then
		...
		self.tic = torch.tic()
		log.info('NoAttentionAgent | step = %d | %.02f%% | speed = %d / sec | η = %.8f',
...
function NoAttentionAgent:saveState()
