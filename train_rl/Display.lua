...
function Display:display(agent, display, step)
  if qt or self.record then
    ...
    -- Record
    if self.record then
      image.save(paths.concat('scratch', self.foldername .. '_' .. string.format('%06d', step) .. '.jpg'), screen)
    end
  end
end
...
function Display:createVideo()
  if self.record then
    ...
    os.execute('ffmpeg -framerate ' .. self.fps .. ' -start_number 1 -i scratch/' .. self.foldername .. '_%06d.jpg -c:v libvpx-vp9 -crf 0 -b:v 0 videos/' .. self.foldername .. '.webm')
