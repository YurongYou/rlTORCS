...
function Body:createBody()
	...
	local valueFunction = nn.Linear(self.hiddenSize / 2, 1)

	local action_mu = nn.Sequential()
	action_mu:add(nn.Linear(self.hiddenSize / 2, self.action_num))

	local action_sigma = nn.Sequential()
	action_sigma:add(nn.Linear(self.hiddenSize / 2, self.action_num))
	action_sigma:add(nn.SoftPlus())
	action_sigma:add(nn.AddConstant(1))

	local coord_mu = nn.Sequential()
	coord_mu:add(nn.Linear(self.hiddenSize / 2, 4))

	local coord_sigma = nn.Sequential()
	coord_sigma:add(nn.Linear(self.hiddenSize / 2, 4))
