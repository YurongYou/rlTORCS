...
function Body:createBody()
	...
	NormalNet:add(nn.ReLU(true))
	self.afterCNN = nn.View(torch.prod(torch.Tensor(getOutputSize(NormalNet, obsInputDim))))
	NormalNet:add(self.afterCNN)
...
-- 	-- theta:copy(weights)
    ...
    for _, smodule in pairs(NormalNet.modules) do
        table.insert(self.CNNmodules, smodule)
    end
    -- delete the last one (nn.View)
    table.remove(self.CNNmodules, #self.CNNmodules)

	return net
end

function Body:getCNN()
    return self.CNNmodules
end

function Body:getAfterCNN()
