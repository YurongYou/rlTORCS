...
function CircularQueue:_init(length, createTensor, tensorSizes)
  ...
  local size = {}
  size[1] = tensorSizes[1]*length
  size[2] = tensorSizes[2]
  size[3] = tensorSizes[3]
  self.buffer = createTensor(torch.LongStorage(size)):zero()
...
function CircularQueue:readAll()
  for i = 1,self.length do
    self.buffer:narrow(1,i,1):copy(self.queue[i])
  end
  return self.buffer
